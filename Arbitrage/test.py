...
def main():
    start_time = time()
    csvfile = open('usdttriangle.csv', 'w', newline='', encoding='UTF8')
    ...
    for ticker in prices:
        ...
        for primary in PRIMARY:
            if pair.endswith(primary):
                ...
                prepared[primary][secondary] = 1 / ask #Want left 
                prepared[secondary][primary] = bid #Want right
    return prepared
...
def find_triangles(prices):
    triangles = []
    starting_coin = 'USDT'
    for triangle in recurse_triangle2(prices, starting_coin, starting_coin):
...
def recurse_triangle2(prices, current_coin, starting_coin, depth_left=3, amount=1.0):
    if depth_left > 0:
        pairs = prices[current_coin]
        for coin, price in pairs.items():
            new_price = (amount * price) * (1.0 - FEE)
            for triangle in recurse_triangle2(prices, coin, starting_coin, depth_left - 1, new_price):
...
def describe_triangle(prices, triangle, result_writer):
    ...
    print(f"{datetime.datetime.now()} {'->'.join(coins):26} {round(price_percentage, 4):-7}% profit")
